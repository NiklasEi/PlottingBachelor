...
homePath="~/plotting/MultiJetBackground/Ht/"


TFileBackground = ROOT.TFile(homePath+"TFileBackgroundPtHt.root", "recreate")
...
nBinsJet = 25 # set number of Bins in Ht for 2D Plots
...
HistSimBackgroundHtNW = ROOT.TH1F( "SimBackgroundHtNW", "SimBackgroundHtNW", 10, 0, 1750)
...
HistSimBackgroundCMetNW = ROOT.TH1F( "SimBackgroundCPredictionMetNW", "SimBackgroundCPredictionMetNW", 18, 0, 900)
...
HistSimBackgroundCHtNW = ROOT.TH1F( "SimBackgroundCHtNW", "SimBackgroundCHtNW", 10, 0, 1750)
HistSimBackgroundCHtNW.SetTitle(Title[0]+" Comparison for QCD and GJet samples")
HistSimBackgroundCHtNW.GetXaxis().SetTitle("Ht(GeV)")
HistSimBackgroundCHtNW.GetYaxis().SetTitle("Events")
...
HistSimBackgroundCPhotonPtNW = ROOT.TH1F( "SimBackgroundCPhotonPtNW", "SimBackgroundCPhotonPtNW", 15, 175, 1900)
HistSimBackgroundCPhotonPtNW.SetTitle(Title[0]+" Comparison for QCD and GJet samples")
HistSimBackgroundCPhotonPtNW.GetXaxis().SetTitle("P_{T}*(GeV)")
HistSimBackgroundCPhotonPtNW.GetYaxis().SetTitle("Events")
...
for name in Names:
	...
	for event in tree:
		...
		if isSignal(event)=="GT":
			...
			if status=="sim":
				HistSimHtPtGT.Fill(event.photons[0].ptStar, event.ht, weight*event.weight)
			if status=="data":
				HistDataHtPtGT.Fill(event.photons[0].ptStar, event.ht)
			...
			if status=="sim":
				HistSimHtPtGL.Fill(event.jetphotons[0].ptStar, event.ht, weight*event.weight)
			if status=="data":
				HistDataHtPtGL.Fill(event.jetphotons[0].ptStar, event.ht)
...
if HistDataHtPtGL.Integral()!=HistDataHtPtGL.GetEntries():
	print "Integral = "+str(HistDataHtPtGL.Integral())+" and Entries = "+str(HistDataHtPtGL.GetEntries())
...
for pt in range(1, nBinsPt+1):# Bin numbers start at 1 first bin (1,1)
	for jet in range(1, nBinsJet+1):
		if HistSimHtPtGT.GetBinContent(pt, jet) < 0.01 and HistSimHtPtGL.GetBinContent(pt, jet) > 1:
			print " Bin ("+str(pt)+","+str(jet)+") hat "+str(HistSimHtPtGT.GetBinContent(pt, jet))+" gt, aber "+str(HistSimHtPtGL.GetBinContent(pt, jet))+" gl"
...
for pt in range(1, nBinsPt+1):# Bin numbers start at 1 first bin (1,1)
	for jet in range(1, nBinsJet+1):
		BinsCount+=1
		if HistSimHtPtGT.GetBinContent(pt, jet)>=0.01:
			countWeightsSim+=1.
		...
		elif HistSimHtPtGT.GetBinContent(pt, jet)<=0.01:
			HistSimHtPtWeightError.SetBinContent(pt, jet, 0) # relative Error on w(i)

		if HistDataHtPtGT.GetBinContent(pt, jet)>=0.01:
			countWeightsData+=1.
		...
		elif HistDataHtPtGT.GetBinContent(pt, jet)<=0.01:
...
for name in Names:
	...
	for event in tree:
		...
		if event.met<100: #need these events to get prediction plots for the controll area at the end: "continue"
			...
			if isSignal(event)=="GL":
				...
				if status=="sim":
					...
					Bin = HistSimHtPtWeight.FindFixBin(event.jetphotons[0].ptStar, event.ht)
					...
					if status=="sim":#name == "QCD_250_500_V03" or name=="QCD_100_250_V09" or name=="QCD_500_1000_V03" or name=="QCD_1000_inf_V03" or name=="GJets_100_200_V09" or name=="GJets_200_400_V03" or name=="GJets_400_inf_V03" or name=="GJets_40_100_V09":
						...
						HistSimBackgroundCMetNW.Fill(event.met, weight*event.weight)
						...
						HistSimBackgroundCHtNW.Fill(event.ht, weight*event.weight)
						...
						HistSimBackgroundCPhotonPtNW.Fill(event.jetphotons[0].ptStar, weight*event.weight)
				if status=="data":

					Bin = HistDataHtPtWeight.FindFixBin(event.jetphotons[0].ptStar, event.ht)
		...
		if isSignal(event)=="GL":
			if status=="sim":
				HistSimHtPtGLSignal.Fill(event.jetphotons[0].ptStar, event.ht, weight*event.weight)
			if status=="data":
				HistDataHtPtGLSignal.Fill(event.jetphotons[0].ptStar, event.ht)
			...
			if status=="sim":
				...
				if status=="sim":#name == "QCD_250_500_V03" or name=="QCD_100_250_V09" or name=="QCD_500_1000_V03" or name=="QCD_1000_inf_V03" or name=="GJets_100_200_V09" or name=="GJets_200_400_V03" or name=="GJets_400_inf_V03" or name=="GJets_40_100_V09":
					if weightGTGL <=0.01:
						...
						HistSimHtPtWeightZeroEvents.Fill(event.jetphotons[0].ptStar, event.ht, weight*event.weight)
			...
			if status=="data":
				...
				if weightGTGL <=0.01:
					...
					HistDataHtPtWeightZeroEvents.Fill(event.jetphotons[0].ptStar, event.ht)
		...
		if isSignal(event)=="GT":
			if status=="sim":
				...
				HistSimHtPtGTSignal.Fill(event.photons[0].ptStar, event.ht, weight*event.weight)
			if status=="data":
				HistDataGTMultiSignal.Fill(event.cleanjets.size(), weight*event.weight)
				HistDataHtPtGTSignal.Fill(event.photons[0].ptStar, event.ht)
...
HistDataBackgroundPredictionMetSys.Write()
...
HistDataBackgroundPredictionHtSys.Write()
...
HistDataBackgroundPredictionPtStarSys.Write()
...
HistSimBackgroundMet.Write()
HistSimBackgroundPredictionMetSys.Write()
...
HistSimBackgroundHt.Write()
HistSimBackgroundPredictionHtSys.Write()
...
HistSimBackgroundPhotonPt.Write()
HistSimBackgroundPredictionPhotonPtSys.Write()

#comparison plots sim in controll region
Canvas1D.cd() # for 1D histos
LMetC = ROOT.TLegend(.6,.75,.9,.9)
HistSimBackgroundCMetNW.SetLineColor(2)
HistSimBackgroundCMetNW.SetMarkerSize(0)
...
HistSimBackgroundCMetNW.Draw("same Ehist")
...
LMetC.AddEntry(HistSimBackgroundCMetNW, "#gamma_{loose}", "l")
...
HistSimBackgroundCHtNW.SetLineColor(2)
HistSimBackgroundCHtNW.SetMarkerSize(0)
...
HistSimBackgroundCHtNW.Draw("same Ehist")
...
LHtC.AddEntry(HistSimBackgroundCHtNW, "#gamma_{loose}", "l")
...
HistSimBackgroundCPhotonPtNW.SetLineColor(2)
HistSimBackgroundCPhotonPtNW.SetMarkerSize(0)
...
HistSimBackgroundCPhotonPtNW.Draw("same Ehist")
...
LPhotonPtC.AddEntry(HistSimBackgroundCPhotonPtNW, "#gamma_{loose}", "l")
